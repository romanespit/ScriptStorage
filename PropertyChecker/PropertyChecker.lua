...
function CheckAndDownloadFiles()
    ...
    if needLoad ~= 0 then
        Logger("Òğåáóåòñÿ çàãğóçêà "..needLoad.." ôàéëîâ. Íà÷èíàş ñêà÷èâàíèå...")
                ...
                if status == dlstatus.STATUS_ENDDOWNLOADDATA then
                    lua_thread.create(function()
                        ...
                        if doesFileExist(v[2]) then
                            Logger("Óñïåøíàÿ çàãğóçêà ôàéëà "..v[2]:match(".+(rmnsptScripts/.+)"))
                            needLoad = needLoad-1
                        end
                    end)
                end
    ...
    if #NeedToLoad > 0 then sms("Çàãğóçêè íåîáõîäèìûõ ôàéëîâ çàâåğøåíû. Ïåğåçàãğóæàåìñÿ...") reloaded = true scr:reload() end
end
------------------------ Script Directories
